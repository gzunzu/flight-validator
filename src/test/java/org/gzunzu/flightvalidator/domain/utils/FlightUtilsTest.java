...
class FlightUtilsTest {
    ...
    void test_getHaversineDistance(final double departureLatitude, final double departureLongitude, final double arrivalLatitude, final double arrivalLongitude, final double expected) {
    ...
    @ParameterizedTest
    @CsvSource({"89,10", "89,-1", "89,0", "89,-90", "0,-90", "-89,-90"})
    void test_isHeadingSouth_true(final double departureLongitude, final double arrivalLongitude) {
        final boolean result = FlightUtils.isHeadingSouth(departureLongitude, arrivalLongitude);
        assertThat(result).isTrue();
    }

    @ParameterizedTest
    @CsvSource({"-89,10", "-89,-1", "-89,0", "-89,90", "0,90", "89,90"})
    void test_isHeadingSouth_false(final double departureLongitude, final double arrivalLongitude) {
        final boolean result = FlightUtils.isHeadingSouth(departureLongitude, arrivalLongitude);
        assertThat(result).isFalse();
    }
